...
from .model import ModelConfig, PUC
...
def train_pipeline(config: TrainConfig, verbose: bool = False) -> Dict[str, Any]:
    ...
    test_dataset = PUCDataset(splits["test"], transform=eval_transform) if splits["test"] else None
    ...
    model = PUC(model_config).to(device)
    base_metadata = {
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        epoch_path = config.output_dir / f"puc_epoch_{epoch:04d}.pt"
        torch.save(epoch_payload, epoch_path)
        _prune_checkpoints(config.output_dir, "puc_epoch_", 10)
        ...
        if score_value > best_f1:
            ...
            best_checkpoint_path = config.output_dir / f"puc_best_epoch{epoch:04d}_f1_{score_value:.4f}.pt"
            best_state["checkpoint_path"] = str(best_checkpoint_path)
            torch.save(best_checkpoint, best_checkpoint_path)
            _prune_checkpoints(config.output_dir, "puc_best_", 10)
...
def run_webcam_inference_onnx(
    onnx_model: Path,
    camera_index: int = 0,
    window_name: str = "PUC Webcam (ONNX)",
    mirror: bool = False,
    ...
    try:
        while True:
            ...
            probs = np.array(outputs[0]).reshape(-1)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PUC training and inference pipeline")
    subparsers = parser.add_subparsers(dest="command", required=True)

    train_parser = subparsers.add_parser("train", help="Train the PUC 4-class phone usage classifier.")
    ...
    webcam_parser.add_argument("--window_name", type=str, default="PUC Webcam")
    ...
    webcam_onnx_parser.add_argument("--model", type=Path, required=True, help="Path to the exported PUC ONNX model.")
    webcam_onnx_parser.add_argument("--camera_index", type=int, default=0, help="OpenCV camera index (default: 0).")
    webcam_onnx_parser.add_argument("--window_name", type=str, default="PUC Webcam (ONNX)")
