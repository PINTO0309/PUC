...
@dataclass
class ModelConfig:
    """Lightweight CNN configuration for the PUC classifier."""
...
class PUC(nn.Module):
    """Compact phone usage classifier that outputs logits."""

    def __init__(self, config: Optional[ModelConfig] = None) -> None:
        super().__init__()
        self.config = config or ModelConfig()
        self.num_classes = max(2, int(getattr(self.config, "num_classes", 4)))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self._head_variant in {"transformer", "mlp_mixer"}:
            ...
            logits = self.head(x)
        if logits.ndim == 2 and logits.shape[1] == 1 and self.num_classes == 1:
