...
class Sample:
    """Container describing a single cropped phone-usage frame."""
...
def _to_label(row: pd.Series) -> int:
    ...
    if label_text:
        label_key = _normalize_label_name(label_text)
        if label_key in CLASS_NAME_TO_ID:
            return CLASS_NAME_TO_ID[label_key]
...
def collect_samples(data_root: Path, logger: Optional[logging.Logger] = None) -> List[Sample]:
    ...
    for idx, row in df.iterrows():
        ...
        label = _to_label(row)
        label_name = str(row.get("label", CLASS_ID_TO_NAME.get(label, "unknown")))
...
class PUCDataset(Dataset):
    """Torch dataset that reads phone usage crops from disk or embedded bytes."""
