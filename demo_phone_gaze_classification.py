...
def should_render_phone_check(
    *,
    interval_active: bool,
    label: str,
    gaze_inout_pass: bool,
) -> bool:
    """Apply the current-frame inout gate after temporal smoothing."""
    return interval_active and bool(label) and gaze_inout_pass
...
def main():
    ...
    if not is_package_installed('onnxruntime'):
        parser.error('onnxruntime is required for the PUC and Gazelle ONNX models.')
    runtime: str = None
    if model_ext == 'onnx':
        runtime = 'onnx'
    ...
    def update_history_status(history: PhoneUsageStateHistory) -> None:
        (
        ...
        history.label = PHONE_CHECK_LABEL if state_interval_judgment else ''
    ...
    while True:
        ...
        positive_hands_by_body = {
            body_index: [
                hand_box
                for hand_box in hand_boxes
                if hand_box.phone_class in (1, 2)
            ]
            for body_index, hand_boxes in hands_by_body.items()
        }
        positive_hands_by_body = {
            body_index: hand_boxes
            for body_index, hand_boxes in positive_hands_by_body.items()
            if hand_boxes
        }
        ...
        for body_box in body_boxes:
            ...
            detection_state = body_box.phone_state == 1
            history.append(detection_state, predicted_class if predicted_class is not None else -1)
            update_history_status(history)
            if should_render_phone_check(
                interval_active=history.interval_active,
                label=history.label,
                gaze_inout_pass=body_box.gaze_inout_pass,
            ):
